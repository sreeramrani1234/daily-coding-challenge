...
-- 1. Retrieve all posts with author’s username
SELECT p.post_id, p.content, p.post_date, u.username as Author_name
FROM Posts p
...
ORDER BY p.post_id, c.comment_date;
...
-- 6. Get a list of all friends of a specific user (user_id = 3)
SELECT 
    u.user_id,
    u.username
FROM Users u
...
-- 7. Retrieve posts that were liked by friends of a given user
SELECT DISTINCT
    p.post_id,
    p.content,
    p.post_date
...
-- 8. Create stored procedure GetUserActivity
DELIMITER //
...
SELECT 
    ...
    COALESCE(COUNT(DISTINCT c.comment_id), 0) AS total_engagement
FROM Users u
LEFT JOIN Posts p ON u.user_id = p.user_id
